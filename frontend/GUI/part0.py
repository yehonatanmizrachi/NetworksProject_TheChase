...
def start_part0(socket, start_game_msg):
    ...
    font_style = tk_font.Font(family=socket.font, size=20)
    ...
    # yes and no buttons
    yes_button = tk.Button(main_frame, text="yes", font='40', bg=socket.button_bg, command=lambda: want_to_play())
    yes_button.place(anchor='n', relx=0.3, rely=0.65, relwidth=0.3, relheight=0.15)

    no_button = tk.Button(main_frame, text="no", font='40', bg=socket.button_bg, command=lambda: dont_want_to_play())
