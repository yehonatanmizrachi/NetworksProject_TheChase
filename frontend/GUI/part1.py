...
def start_part1(socket):
    ...
    def choose_option(current_msg):
        ...
        op1_button = tk.Button(options_frame, text="start at location 3 with your current money", font='40', bg=socket.button_bg, command=lambda: init_location(1))
        op1_button.place(anchor='n', relx=0.5, rely=0.4, relwidth=0.8, relheight=0.15)

        op2_button = tk.Button(options_frame, text="start at location 2 with double money", font='40', bg=socket.button_bg, command=lambda: init_location(2))
        op2_button.place(anchor='n', relx=0.5, rely=0.6, relwidth=0.8, relheight=0.15)

        op3_button = tk.Button(options_frame, text="start at location 4 with half money", font='40', bg=socket.button_bg, command=lambda: init_location(3))
    ...
    font_style = tk_font.Font(family=socket.font, size=18)
